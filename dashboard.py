...
def make_forecast(model, forecast_df: pd.DataFrame, last_aqi: float):
    ...
    for _, row in forecast_df.iterrows():
        ...
        # hist_df is stored in UTC; Open-Meteo timestamps are PKT (UTC+5)
        # Subtract 5h to align forecast time with UTC-stored history
        ts_utc = ts_naive - pd.Timedelta(hours=5)

        def get_lag(hours):
            lag_time = ts_utc - pd.Timedelta(hours=hours)
            if not hist_df.empty:
                past = hist_df[hist_df["timestamp"] <= lag_time]
                if not past.empty:
                    return past["aqi"].iloc[-1]
        ...
        def get_rolling(hours):
            end_time   = ts_utc - pd.Timedelta(hours=1)
            start_time = ts_utc - pd.Timedelta(hours=hours)
            if not hist_df.empty:
                window = hist_df[
                    (hist_df["timestamp"] >= start_time) &
                    (hist_df["timestamp"] <= end_time)
