...
# Pulled from preprocessing.py directly so the dashboard can never drift out of
# sync with the feature set the model was actually trained on (previously a
# hand-copied list here that would silently go stale the next time features
# were changed in preprocessing.py).
...
def make_forecast(model, forecast_df, last_aqi):
    ...
    if not hist_df.empty:
        ...
        # Never let forecast lag/rolling features draw on fabricated backfill
        # rows — keeps this in sync with training_pipeline.py's own filter.
        if "data_source" in hist_df.columns:
            hist_df = hist_df[hist_df["data_source"] != "synthetic_backfill"]
    rows = []
    for _, row in forecast_df.iterrows():
        ts   = row["timestamp"]
        ts_n = (ts.tz_convert("UTC").tz_localize(None)
        ...
        raw_pred = float(model.predict(pd.DataFrame([feats])[FEATURE_COLS])[0])
        # Trust the model's actual output — the previous ±45%/+55% clamp around
        # today's AQI was a band-aid for the old synthetic-data problem. Now
        # that training data is real and honestly scored, clamping this tightly
        # would silently suppress genuine predicted swings (e.g. a storm
        # clearing the air, or a smog event building) without any indication
        # to the viewer that it happened. Keep only a hard physical sanity
        # bound on the AQI scale itself.
        pred = max(0, min(500, round(raw_pred, 1)))
        rows.append({"timestamp": ts_n, "predicted_aqi": pred, "temp": temp,
                     "humidity": hum, "wind_speed": ws,
                     "precipitation": row.get("precipitation", 0),
                     "date": ts_n.date(), "hour": hour})
